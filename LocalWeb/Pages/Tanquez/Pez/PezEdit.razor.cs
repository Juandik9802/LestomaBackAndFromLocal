...
namespace LocalWeb.Pages.Tanquez.Pez
{
    public partial class PezEdit
    {
        ...
        protected async override Task OnInitializedAsync()
        {
            ...
            try
            {
                var responseHttp = await Repository.GetAsync<ClsMPez>($"/api/pez/{Id}");
                if (!responseHttp.Error)
                {
                    Pez = responseHttp.Responce;
                }
                else
                {
        ...
        {
            var responseHttp = await Repository.PutAsync("api/pez", Pez);
            if (responseHttp.Error)
            {
                var message = await responseHttp.GetErrorMessageAsync();
